...
void servidor::conexion_nueva(){
    ...
    ui->lineEdit->setText("Connected Client...");
    qDebug() << "Reservacion de memoria de 10MB";
}
...
void servidor::leer_socketcliente() {
    if(tcpcliente[0]->bytesAvailable() > 0){
        ...
        qDebug() << "NAME = " << nameJson.toString();

        foreach(const QString& key, jsonObject.keys()) {
                QJsonValue value = jsonObject.value(key);
                qDebug() << "Key = " << key << ", Value = " << value.toString();
            }
        data->filtro("int");   // si es un int
    }
    else{
